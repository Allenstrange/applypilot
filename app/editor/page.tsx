  ...
  if (!analysis || !draftCV) {
    return (
      <div className="p-12 text-center text-gray-500">
        ...
        <Link href="/analyze" className="btn-primary text-white px-4 py-2 rounded-lg text-sm mt-4 inline-block">
          Analyse a Job First
        </Link>
      </div>
    );
  ...
  return (
    <div className="p-8">
      <div className="flex items-start justify-between mb-6 flex-wrap gap-3">
        <div>
          <h1 className="text-2xl font-bold text-white">Editing Room</h1>
        ...
        {TABS.map(({ id, label, icon: Icon }) => (
          <button
            ...
            className={`relative px-4 py-2 text-sm font-medium flex items-center gap-2 ${
              tab === id ? "text-amber-300" : "text-gray-400 hover:text-white"
            }`}
          >
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
      ...
      <div className="space-y-4">
                ...
                <div className="space-y-2">
                  {exp.bullets.split("\n").map((bullet, j) => (
                    <div key={j} className="relative pr-2">
                      <textarea
                        rows={2}
                        className="w-full px-2 py-1 rounded text-xs bg-black/30"
                        value={bullet}
                      ...
                      <button
                        ...
                        className="absolute top-1 right-1 text-[10px] px-1.5 py-0.5 rounded bg-gradient-to-r from-amber-500 to-pink-500 text-white"
                      >
                        ✨ AI
                      </button>
                      {openMenu === `${i}-${j}` ? (
                  ...
                  <button type="button" onClick={() => updateExp(i, { bullets: exp.bullets + "\nNew bullet point here" })} className="text-xs text-amber-400 hover:text-amber-300">
                    + Add bullet
                  </button>
                </div>
              </div>
        ...
        <button type="button" onClick={() => { exportCVPDF(draftCV); toast("✓ CV downloaded"); }} className="btn-primary text-white px-4 py-2 rounded-lg text-sm flex items-center gap-2">
      ...
      <div>
        <div className="glass rounded-xl p-4 mb-3 sticky top-4 z-10">
          <div className="text-xs text-gray-500 uppercase mb-1">Live Match Rate</div>
    ...
    >
      {data ? (
        <div className="card rounded-xl p-6 space-y-4 leading-relaxed">
          ...
          <div className="font-semibold text-white"><Highlight text={data.subjectLine} keywords={data.keywords} /></div>
          <p className="text-gray-200"><Highlight text={data.salutation} keywords={data.keywords} /></p>
          {data.body.map((p, i) => (
            <p key={i} className="text-gray-300"><Highlight text={p} keywords={data.keywords} /></p>
          ))}
          <p className="text-gray-300"><Highlight text={data.closing} keywords={data.keywords} /></p>
          <p className="text-gray-200 whitespace-pre-line"><Highlight text={data.signOff} keywords={data.keywords} /></p>
        </div>
      ) : null}
    </OutputShell>
  );
    ...
    >
      {data ? (
        <div className="card rounded-xl p-6 space-y-4">
          <KeywordBadges keywords={data.keywords} />
          <h3 className="text-lg font-bold text-white"><Highlight text={data.targetHeadline} keywords={data.keywords} /></h3>
          <p className="text-gray-300 leading-relaxed"><Highlight text={data.introSummary} keywords={data.keywords} /></p>
          <ul className="space-y-2">
            {data.bulletPoints.map((b, i) => (
              <motion.li key={i} initial={{ opacity: 0, x: -8 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: i * 0.05 }} className="text-gray-300 flex gap-2">
        ...
        <div className="space-y-3">
          ...
          {data.questions.map((q, i) => (
            <motion.div key={i} initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.04 }} className="card rounded-xl p-5">
              <div className="font-semibold text-white mb-2">Q{i + 1}. <Highlight text={q} keywords={data.keywords} /></div>
              {data.answerFormulas[i] ? (
                <div className="text-sm text-gray-300 mb-2"><span className="text-amber-400 font-medium">How to frame: </span><Highlight text={data.answerFormulas[i]} keywords={data.keywords} /></div>
              ) : null}
              {data.coachTips[i] ? (
                <div className="text-xs text-gray-500"><span className="text-indigo-400 font-medium">Coach tip: </span><Highlight text={data.coachTips[i]} keywords={data.keywords} /></div>
              ) : null}
            </motion.div>
          ))}
        </div>
    ...
    >
      {data ? (
        <div className="card rounded-xl p-6 space-y-3">
          <KeywordBadges keywords={data.keywords} />
          <div className="text-xs text-gray-500 uppercase">Subject</div>
          <div className="font-semibold text-white"><Highlight text={data.subject} keywords={data.keywords} /></div>
          <div className="text-gray-300 whitespace-pre-line leading-relaxed"><Highlight text={data.message} keywords={data.keywords} /></div>
        </div>
      ) : null}
    </OutputShell>
  );
    ...
    <div>
      <div className="flex items-center justify-between mb-4 flex-wrap gap-2">
        ...
        <div className="flex gap-2">
          {actions}
          <button type="button" onClick={onGenerate} disabled={busy} className="btn-primary text-white px-4 py-2 rounded-lg text-sm flex items-center gap-2">
      ...
      {hasData ? children : (
        <div className="card rounded-xl p-12 text-center text-gray-500">
          Click <span className="text-amber-300">Generate</span> to create tailored, keyword-rich content.
        </div>
      )}
    </div>
  );
...
function MenuItem({ onClick, children }: { onClick: () => void; children: React.ReactNode }) {
  return (
    <button type="button" onClick={onClick} className="block w-full text-left px-3 py-2 rounded hover:bg-amber-500/20 text-gray-200">
      {children}
    </button>
  );
}
