    ...
    <div className="mt-4 rounded-lg border border-[var(--border)] bg-[var(--surface-2)] p-4" data-testid="jd-keywords">
      <div className="flex items-center justify-between mb-3">
        ...
        <span className="text-[11px] text-slate-500 dark:text-slate-400">
          Local · no AI · weave these into your CV
        </span>
      </div>
              ...
              {g.keywords.map((k) => (
                <span
                  ...
                  className={`inline-flex items-center gap-1 rounded-full border px-2 py-0.5 text-xs ${
                    k.required > 1
                      ? "border-[var(--brand)]/40 bg-[color-mix(in_srgb,var(--brand)_10%,transparent)] text-[var(--brand)] font-medium"
                      : "border-[var(--border)] bg-[var(--surface)] text-slate-700 dark:text-slate-200"
                  }`}
                >
                  {k.token}
                  {k.count > 1 ? (
