  ...
  return (
    <div className="space-y-6">
      ...
      <div className="card rounded-xl p-6">
        ...
        <p className="text-xs text-slate-500 mb-3">Comma-separated. Used for keyword matching.</p>
      ...
      <div className="card rounded-xl p-6">
        ...
        <div className="space-y-4">
          {profile.experience.length === 0 ? (
            <div className="text-slate-500 text-sm text-center py-4">No experience added yet.</div>
          ) : (
            profile.experience.map((exp, i) => (
              <div key={i} className="p-4 rounded-lg bg-slate-50 border border-slate-200">
                <div className="flex justify-between items-start mb-3">
                  ...
                  <button type="button" onClick={() => removeExp(i)} className="text-red-600 text-xs hover:text-red-700">Remove</button>
                </div>
                ...
                <textarea rows={3} className="w-full px-3 py-2 rounded-lg text-sm mb-2" placeholder="Key responsibilities (one per line)" value={exp.bullets} onChange={(e) => updateExp(i, { bullets: e.target.value })} />
                <input className="w-full px-3 py-2 rounded-lg text-sm" placeholder="Tools used (comma separated)" value={exp.tools} onChange={(e) => updateExp(i, { tools: e.target.value })} />
              </div>
            ))
          )}
        </div>
      ...
      <div className="card rounded-xl p-6">
        ...
        <div className="space-y-3">
          {profile.education.length === 0 ? (
            <div className="text-slate-500 text-sm text-center py-4">No education added yet.</div>
          ) : (
