...
import { resolveTemplate, DEFAULT_SECTION_ORDER, type ResolvedTemplate, type StyleOverrides } from "./templates";
import { parseBullets, parseLines } from "./resumeFormat";
...
export function exportResumePDF(
  ...
  singleColumnPDF(profile, tpl, order);
}

function singleColumnPDF(profile: Profile, tpl: ResolvedTemplate, order: SectionKey[]) {
  ...
  order.forEach((key) => sections[key]());
