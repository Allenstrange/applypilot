...
export default function OutreachHub() {
  ...
  useEffect(() => {
    ...
    ]).then(async ([{ data: td }, { data: md }]) => {
      const mapped = (td ?? []).map(mapTemplate)
      if (mapped.length === 0) await seedTemplates()
      else setTemplates(mapped)
      setMessages((md ?? []).map(mapMessage))
    })
  }, [])

  async function seedTemplates() {
    const toInsert = STARTER_TEMPLATES.map(t => ({ ...t, id: generateId(), created_at: new Date().toISOString() }))
    const { data } = await supabase.from('outreach_templates').insert(toInsert).select()
    if (data) setTemplates(data.map(mapTemplate))
  }

  function mapTemplate(r: Record<string, unknown>): OutreachTemplate {
    return { id: r.id as string, name: r.name as string, type: r.type as OutreachTemplate['type'], subject: r.subject as string, body: r.body as string, tags: (r.tags ?? []) as string[], createdAt: r.created_at as string }
  }
  function mapMessage(r: Record<string, unknown>): OutreachMessage {
    return { id: r.id as string, applicationId: r.application_id as string, templateId: r.template_id as string, recipientName: r.recipient_name as string, recipientEmail: r.recipient_email as string, channel: r.channel as OutreachMessage['channel'], subject: r.subject as string, body: r.body as string, status: r.status as OutreachMessage['status'], sentDate: r.sent_date as string, createdAt: r.created_at as string }
  }
  ...
  async function saveTemplate() {
    ...
    const payload = { name: templateForm.name, type: templateForm.type, subject: templateForm.subject, body: templateForm.body, tags: templateForm.tags.split(',').map(s => s.trim()).filter(Boolean) }
    if (editingTemplate) {
      const { data } = await supabase.from('outreach_templates').update(payload).eq('id', editingTemplate.id).select().single()
      if (data) setTemplates(prev => prev.map(t => t.id === editingTemplate.id ? mapTemplate(data) : t))
    } else {
      const { data } = await supabase.from('outreach_templates').insert({ ...payload, id: generateId(), created_at: new Date().toISOString() }).select().single()
      if (data) setTemplates(prev => [mapTemplate(data), ...prev])
    }
  ...
  function updateTokens(form: typeof composeForm) {
    ...
    const replacements: Record<string, string> = {
      '{{recruiter_name}}': form.recipientName || '[Name]',
      '{{your_name}}': 'You',
    }
  ...
  async function sendMessage() {
    ...
    const { data } = await supabase.from('outreach_messages').insert({
      id: generateId(), recipient_name: composeForm.recipientName, recipient_email: composeForm.recipientEmail,
      channel: composeForm.channel, subject: composeForm.subject, body: previewBody,
      template_id: selectedTemplate?.id, status: 'sent', sent_date: new Date().toISOString(), created_at: new Date().toISOString(),
    }).select().single()
    if (data) setMessages(prev => [mapMessage(data), ...prev])
    setShowComposeModal(false)
    addToast('Message logged as sent', 'success')
  }

  async function updateMessageStatus(id: string, status: OutreachMessage['status']) {
  ...
  return (
    <div className="p-6 max-w-6xl mx-auto">
      <div className="flex items-center justify-between mb-6">
        ...
        <button onClick={tab === 'templates' ? openNewTemplate : () => { if (templates.length > 0) openCompose(templates[0]) }}
            ...
            <div key={t.id} className="card hover:border-navy-600 transition-colors">
              <div className="flex items-start justify-between mb-2">
                ...
                <div className="flex gap-1">
                  ...
                  <button onClick={() => { const c = { ...t, id: generateId(), name: `${t.name} (copy)`, createdAt: new Date().toISOString() }; supabase.from('outreach_templates').insert({ id: c.id, name: c.name, type: c.type, subject: c.subject, body: c.body, tags: c.tags, created_at: c.createdAt }).select().single().then(({ data }) => { if (data) setTemplates(prev => [mapTemplate(data), ...prev]) }) }} className="text-slate-600 hover:text-slate-400 p-1"><Copy size={13} /></button>
        ...
        <>
          {sent > 0 && (
            <div className="card mb-4 flex items-center gap-6">
              <div className="text-center">
                <p className="text-2xl font-bold text-slate-100">{sent}</p>
                <p className="text-xs text-slate-500">Sent</p>
              </div>
              <div className="text-center">
                <p className="text-2xl font-bold text-green-400">{replied}</p>
                <p className="text-xs text-slate-500">Replied</p>
              </div>
              <div className="text-center">
                <p className="text-2xl font-bold text-teal-400">{sent > 0 ? Math.round((replied / sent) * 100) : 0}%</p>
          ...
          <div className="card p-0 overflow-hidden">
            <table className="w-full text-sm">
              ...
              <tbody>
                {messages.length === 0 ? <tr><td colSpan={5} className="px-4 py-8 text-center text-slate-600 text-xs">No messages sent yet</td></tr> : messages.map(m => (
                  <tr key={m.id} className="border-b border-navy-800 hover:bg-navy-700/30">
                    <td className="px-4 py-3">
                      <p className="text-slate-200 font-medium">{m.recipientName}</p>
                      {m.recipientEmail && <p className="text-xs text-slate-500">{m.recipientEmail}</p>}
                    </td>
        ...
        <div className="fixed inset-0 z-40 flex items-center justify-center bg-black/70 p-4">
          <div className="bg-navy-800 border border-navy-600 rounded-2xl w-full max-w-xl max-h-[90vh] overflow-y-auto">
            ...
            <div className="p-5 space-y-3">
              ...
              <div><label className="form-label">Body (use &#123;&#123;tokens&#125;&#125; for personalization)</label><textarea rows={8} className="w-full resize-none font-mono text-xs" value={templateForm.body} onChange={e => setTemplateForm(f => ({ ...f, body: e.target.value }))} /></div>
