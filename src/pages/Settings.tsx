...
export default function Settings() {
  ...
  function handleProviderChange(p: AIConfig['provider']) {
    setProvider(p); setModel(MODEL_OPTIONS[p]?.[0] ?? 'custom'); setTestResult(null)
  }
    ...
    <div className="p-6 max-w-2xl mx-auto">
      ...
      <div className="card space-y-5">
        ...
        <div>
          ...
          <div className="relative">
            <input type={showKey ? 'text' : 'password'} className="w-full pr-10" value={apiKey} onChange={e => setApiKey(e.target.value)}
        ...
        {testResult && (
          <div className={`flex items-start gap-3 p-4 rounded-xl border ${testResult.success ? 'bg-green-500/10 border-green-500/30' : 'bg-red-500/10 border-red-500/30'}`}>
            {testResult.success ? <CheckCircle2 size={18} className="text-green-400 flex-shrink-0 mt-0.5" /> : <XCircle size={18} className="text-red-400 flex-shrink-0 mt-0.5" />}
            <div>
      ...
      <div className="card mt-4">
        ...
        <div className="text-xs text-slate-500 space-y-1">
          <p>AI-powered job application platform built with React, Vite, and Tailwind CSS.</p>
          <p>All data is stored locally in your browser (localStorage). No account or backend required.</p>
          <p className="text-slate-600 mt-3">v2.0.0 — Vite + React 18 + Tailwind + Recharts</p>
        </div>
      </div>
    </div>
  )
